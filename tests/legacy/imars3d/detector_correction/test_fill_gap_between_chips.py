...
def test_fill_gap_with_interpolation():
    ...
    fill_gap.correct_gap(fill_method="interpolation_x_axis", width_range=[2, 3], height_range=[0, 2])
    ...
    fill_gap.correct_gap(fill_method="interpolation_x_axis", width_range=[2, 3], height_range=[3, 5])
    expected_new_detector[3:5, 2] = 3.33333333
    expected_new_detector[3:5, 3] = 3.666666667
    assert np.allclose(expected_new_detector, fill_gap.detector_data)

    fill_gap.correct_gap(fill_method="interpolation_y_axis", width_range=[0, 3], height_range=[2, 2])
    expected_new_detector[2, 0:2] = 2
    expected_new_detector[2, 2] = 2.33333333
    assert np.allclose(expected_new_detector, fill_gap.detector_data)

    fill_gap.correct_gap(fill_method="interpolation_y_axis", width_range=[3, 6], height_range=[2, 2])
...
def test_fill_gap_with_mean():
    ...
    fill_gap = detector_correction.fill_gap_between_chips.FillGapBetweenChips(detector_data=new_detector)
    fill_gap.correct_gap(fill_method="mean_x_axis", width_range=[2, 3], height_range=[0, 2])
    expected_new_detector[0:2, 2] = 1.5
    expected_new_detector[0:2, 3] = 1.5
    assert np.allclose(expected_new_detector, fill_gap.detector_data)

    fill_gap.correct_gap(fill_method="mean_x_axis", width_range=[2, 3], height_range=[3, 5])
    expected_new_detector[3:5, 2] = 3.5
    expected_new_detector[3:5, 3] = 3.5
    assert np.allclose(expected_new_detector, fill_gap.detector_data)

    fill_gap.correct_gap(fill_method="mean_y_axis", width_range=[0, 3], height_range=[2, 2])
    expected_new_detector[2, 0:2] = 2
    expected_new_detector[2, 2] = 2.5
    assert np.allclose(expected_new_detector, fill_gap.detector_data)

    fill_gap.correct_gap(fill_method="mean_y_axis", width_range=[3, 6], height_range=[2, 2])
    expected_new_detector[2, 4:6] = 3
