...
install_chips_in_new_detector = detector_correction.install_chips_in_new_detector.InstallChipsInNewDetector(
    new_detector_height=new_detector_height, new_detector_width=new_detector_width
)
...
install_chips_in_new_detector.put_chip_in_place(
    chip_data=chip2, y_position=chip2_offset.y_offset, x_position=chip2_offset.x_offset + chip_width
)


chip3_offset = chips_offset.chips.chip3
install_chips_in_new_detector.put_chip_in_place(
    chip_data=chip3, y_position=chip3_offset.y_offset + chip_height, x_position=chip3_offset.x_offset
)


chip4_offset = chips_offset.chips.chip4
install_chips_in_new_detector.put_chip_in_place(
    chip_data=chip4, y_position=chip4_offset.y_offset + chip_height, x_position=chip4_offset.x_offset + chip_width
)
...
if method == "interpolation":
    ...
    fill_gap.correct_gap(
        fill_method="interpolation_x_axis",
        width_range=[chip_width, chip_width + chip2_offset.x_offset],
        height_range=[chip2_offset.y_offset, chip_height],
    )
    fill_gap.correct_gap(
        fill_method="interpolation_x_axis",
        width_range=[chip3_offset.x_offset + chip_width, chip3_offset.x_offset + chip_width + chip4_offset.x_offset],
        height_range=[chip4_offset.y_offset + chip_height, 2 * chip_height + chip4_offset.y_offset],
    )

    # horizontal strips
    fill_gap.correct_gap(
        fill_method="interpolation_y_axis",
