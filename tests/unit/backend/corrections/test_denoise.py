...
def test_denoise(mock_denoise_by_bilateral, mock_denoise_by_median, fake_noisy_image):
    _, img_noisy = fake_noisy_image
    # test call to median method
    denoise(arrays=img_noisy, method="median")
    mock_denoise_by_median.assert_called_once()
    # test call to bilateral method
    denoise(arrays=img_noisy, method="bilateral")
    mock_denoise_by_bilateral.assert_called_once()
    # test call to invalid method
    with pytest.raises(ValueError):
        denoise(arrays=img_noisy, method="invalid")


def test_denoise_by_median(fake_noisy_image):
    _, img_noisy = fake_noisy_image
...
def test_denoise_by_bilateral(fake_noisy_image):
    # NOTE:
...
def test_denoise_by_bilateral_2d(fake_noisy_image):
    _, img_noisy = fake_noisy_image
    noise_level_noisy = measure_noiseness(img_noisy)
