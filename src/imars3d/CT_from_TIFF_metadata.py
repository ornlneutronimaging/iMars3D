...
"""
    ...
    def __init__(
        ...
        self.logger = logging.getLogger("CT_from_TIFF_metadata")
    ...
    def _find_OB_DF_files(self, type, subdir, fail_on_not_found=True):
        ...
        for p in candidates:
            ...
            if not np.isclose(et, self.exposure_time):
                self.logger.debug(
                    "%s file %s was exposed %s seconds, but CT was exposed %s seconds"
        ...
        if len(before_CT) > 0:
            ...
            if len(after_CT) > 0:
                ...
                msg = (
                    "No files within one day of CT measurement. will use %s files after one day of CT measurement"
                    % type
                )
                warnings.warn(msg)
    ...
    def _getCTfiles(self):
        ...
        for index, (angle, path) in enumerate(angle_file_list):
            ...
            if index % frame_size != frame_size - 1:
                continue
            # get all frames
