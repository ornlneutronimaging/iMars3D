...
"""
    ...
    def __init__(
        ...
        self.obs = obs
        self.angles = angles
        self.theta = np.array(angles) * np.pi / 180.0
    ...
    def preprocess(self, workdir=None, outdir=None):
        ...
        if self.gamma_filter is not False:
            ...
            gamma_filtered = gamma_filter(
                ct_series, workdir=os.path.join(workdir, "gamma-filter"), parallel=self.parallel_preprocessing
            )
        else:
            # no filtering
    ...
    def recon(
        ...
        remove_rings_at_sinograms=None,
        smooth_recon=None,
        remove_rings=None,
        **kwds
    ):
        ...
        if crop_window is None:
            ...
            cropped = self.crop(pre, left=xmin, right=xmax, top=ymin, bottom=ymax)
        ...
        pre = if_corrected
        if tilt is None:
            tilt_corrected, tilt = self.correctTilt_loop(pre, workdir=workdir)
        else:
        ...
        if smooth_recon:
            ...
            recon = self.r.sm_recon = smooth(
                recon,
                workdir=os.path.join(self.outdir, "smoothed"),
                parallel=self.parallel_preprocessing,
    ...
    def removeRings(self, reconned=None, outdir=None, outfilename_template=None, **kwds):
        ...
        corrected_ifs = io.ImageFileSeries(
            ...
            name="Ring-artifact-directly-removed reconstruction",
            mode="w",
        )
        # process in chunks
        N = len(reconned)
    ...
    def correctTilt_loop(self, pre, workdir):
        ...
        for i in range(NROUNDS):
            tilt_corrected, tilt = i3.correct_tilt(
                pre,
                workdir=os.path.join(workdir, "tilt-correction-%s" % i),
                max_npairs=None,
                parallel=self.parallel_preprocessing,
            )
            if self.clean_intermediate_files == "on_the_fly":
    ...
    def crop(self, series, left=None, right=None, top=None, bottom=None):
        ...
        return crop(series, workdir=os.path.join(self.workdir, "crop"), box=box, parallel=self.parallel_preprocessing)
    ...
    def smooth(self, series, outname="smoothed", **kwds):
        ...
        return smooth(
            series, workdir=os.path.join(self.workdir, outname), parallel=self.parallel_preprocessing, **kwds
        )
    ...
    def reconstruct(
        ...
        **kwds
        ...
        angles, sinograms = i3.build_sinograms(
            ct_series,
            workdir=os.path.join(workdir, "sinogram"),
            parallel=self.parallel_preprocessing,
        ...
        if explore_rot_center:
            ...
            if not os.path.exists(dpath):  # skip if already done
                tomopy.write_center(
                    proj.copy(), theta, cen_range=[X // 2 - DEVIATION, X // 2 + DEVIATION, 1.0], dpath=dpath
                )
        ...
        if rot_center is None:
            ...
            rot_center = find_rot_center.find(ct_series, workdir=os.path.join(workdir, "find-rot-center"))
        ...
        if remove_rings_at_sinograms:
            ...
            self.r.rar_sino = sinograms = i3.ring_artifact_removal_Ketcham(
                sinograms,
                workdir=os.path.join(workdir, "rar_sinograms"),
                parallel=self.parallel_preprocessing,
                **remove_rings_at_sinograms
            )
            recon = i3.reconstruct(
