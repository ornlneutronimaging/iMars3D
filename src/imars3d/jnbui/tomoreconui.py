...
class StartButtonPanel(base.Panel):
    ...
    def __init__(self, context):
        ...
        explanation = ipyw.Label(
            "Do you wish to start from scratch or use a previous reconstruction configuration?",
...
class FileSelectPanel(base.Panel):
    ...
    def __init__(self, context):
        ...
        """
        self.context = context
        self.fsp = flselect.FileSelectorPanel(
            "Choose a .pkl file to use for configuration", start_dir=context.ui_config.start_directory
        )
...
class DFPanel(base.DFPanel):
    ...
    def nextStep(self):
        ...
        os.makedirs(self.context.config.outdir)
        os.chdir(self.context.config.outdir)
        assert os.getcwd() == self.context.config.outdir, "Current directory {} is not selected outdir {}".format(
            os.getcwd(), self.context.config.outdir
        )
...
class MainUIPanel(base.Panel):
    ...
    def onRecon(self, event):
        ...
        def run_recon():
            ...
            recon_slider = ImgSlider.ImageSlider(context.ct.r.reconstructed, self.width, self.height)
