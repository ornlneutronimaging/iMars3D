...
def filter_parallel_onenode(ct_series, output_img_series, desc, filter_one, **kwds):
    """
    ...
    if not os.path.exists(dir):
        if rank == 0:
            ...
            for i in range(WAIT_COUNT):
                ...
                continue
            if not created:
                raise IOError("Waited %s seconds, %s still not created" % (WAIT_COUNT * WAIT_SECONDS, dir))
    #
