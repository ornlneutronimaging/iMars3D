...
class UseCenters:
    ...
    def __call__(self, img0, img180):
        ...
        return 0.7 * np.arctan(slope) * 180.0 / np.pi, 1.0
...
def computeTilt(img0, img180, workdir=None, **kwds):
    centers = np.array([item for item in iterCenters(img0, img180, workdir=workdir, **kwds)])
...
def iterCenters(img0, img180, workdir=None, sigma=3, maxshift=20):
    ...
    for i, (line0, line180) in enumerate(zip(edge0, edge180)):
        ...
        # print i,c
        if c > (line0.size - maxshift) / 2.0 + maxshift // 40.0:  # remove edge cases
            yield i, c
        continue
    return
