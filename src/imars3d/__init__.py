...
@dec.timeit
def build_sinograms(ct_series, workdir="work", parallel=True, parallel_nodes=None):
...
def ring_artifact_removal_Ketcham(sinograms, workdir="work", parallel=True, filename_template=None, **kwds):
    if filename_template is None:
        filename_template = "rar_sino_%i.tiff"
    rar_sino = io.ImageFileSeries(
        os.path.join(workdir, filename_template), identifiers=sinograms.identifiers, name="RAR_Sinogram", mode="w"
    )
