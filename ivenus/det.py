...
"""
Module for detector corrections.
"""
...
class RetrieveMCPChipsOffset(object):
    ...
    will retrieve the MCP xoffset and yoffset of the
    4 various chips.
    """
    ...
    def retrieve_all_offset(self):
        '''
        Retrieve from the yml file the various xoffset,
        yoffset of the chips.
        '''
        detector_config = config.load_yaml_config(self.config_file_name)
    ...
    def retrieve_chip_offset(self, chip):
        '''
        passing the chip # object structure, retrieve the
        x and y value of the offset.
        '''
        _chip = ChipOffset()
    ...
    def get_detector_new_width_offset(self):
        ...
        if chip2_x_offset <= 0:
            ...
            if chip4_x_offset <= 0:
                ...
                if chip3_x_offset <= 0:
                    max_offset = max(
                        [abs(chip4_x_offset), abs(chip2_x_offset)])
    ...
    def get_detector_new_height_offset(self):
        ...
        if chip2_y_offset <= 0:
            ...
            if chip4_y_offset <= 0:
                ...
                if chip3_y_offset <= 0:
                    max_offset = max(
                        [abs(chip4_y_offset), abs(chip2_y_offset)])
